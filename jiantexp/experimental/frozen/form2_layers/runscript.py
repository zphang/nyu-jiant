...
from jiant.shared.runner import save_model_with_metadata
...
class RunConfiguration(zconf.RunConfig):
    ...
    do_save_init = zconf.attr(action="store_true")
    ...
    #
    pooler_config = zconf.attr(type=str, required=True)
...
def setup_runner(
    ...
    pooler_config = py_io.read_json(args.pooler_config)
    named_parameters = freeze_layers(jiant_model.named_parameters(), pooler_config)
...
def run_loop(args: RunConfiguration, checkpoint=None):
    ...
    with quick_init_out.log_writer.log_context():
        ...
        if args.do_save_init:
            save_model_with_metadata(
                model_or_state_dict=runner.model,
                output_dir=args.output_dir,
                file_name="init_model",
            )
        if args.do_train:
...
def freeze_layers(named_parameters, pooler_config):
    trained_parameters = []
    for n, p in named_parameters:
        for layer in pooler_config["frozen_layer_ls"]:
            if f"layer.{layer}." in n:
                p.requires_grad = False
                continue
            if pooler_config["freeze_embeddings"] and ".embeddings." in n:
                p.requires_grad = False
                continue
        trained_parameters.append((n, p))
