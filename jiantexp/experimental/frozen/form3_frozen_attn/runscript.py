...
import jiantexp.experimental.frozen.form3_frozen_attn.runner as frozen_attn_runner
...
class RunConfiguration(zconf.RunConfig):
    ...
    frozen_model_path = zconf.attr(type=str, required=True)
    frozen_model_load_mode = zconf.attr(default="from_transformers", type=str)
...
def setup_runner(
    ...
    with distributed.only_first_process(local_rank=args.local_rank):
        ...
        # Testing out
        freeze_attn_parameters = []
        for k, v in jiant_model.named_parameters():
            if ("self.query" in k) or ("self.key" in k) or ("self.value" in k):
                freeze_attn_parameters.append((k, v))
        torch_utils.set_requires_grad(freeze_attn_parameters, requires_grad=False)
        jiant_model.to(quick_init_out.device)
    with distributed.only_first_process(local_rank=args.local_rank):
        # load the model
        frozen_model = jiant_model_setup.setup_jiant_model(
            model_type=args.model_type,
            model_config_path=args.model_config_path,
            tokenizer_path=args.model_tokenizer_path,
            task_dict=jiant_task_container.task_dict,
        ...
        jiant_model_setup.delegate_load_from_path(
            jiant_model=frozen_model, weights_path=args.frozen_model_path, load_mode=args.frozen_model_load_mode
        )
        frozen_model.to(quick_init_out.device)
        frozen_model.eval()
        torch_utils.set_requires_grad(frozen_model.named_parameters(), requires_grad=False)
    ...
    runner = frozen_attn_runner.FrozenAttnRunner(
